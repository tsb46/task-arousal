...
# dataclass for storing RRR prediction results
@dataclass
class RRRParams:
    n_components: int
    physio_lags: int
...
class RRRResults:
    ...
    rrr_lm: Ridge
    rrr_pca: PCA
    rrr_pca_scores: np.ndarray
    rrr_beta_proj: np.ndarray
    rrr_beta_rrr: np.ndarray
    all_reg_cols: List[str]
...
class RRREventPhysioModel:
    """
    Reduced Rank Regression analysis of task events and physio signals regressed onto functional 
    MRI time courses. 
    ...
    def fit(
        ...
        """
        fit RRR model of combined event and physio lag spline basis
        ...
        # fit RRR regression model
        self.rrr_lm = Ridge(alpha=1.0)
        self.rrr_lm.fit(
            all_regs_concat,
            fmri_concat
        )
        # get estimated scores
        rrr_lm_scores = self.rrr_lm.predict(all_regs_concat)
        # fit PCA to outcome data to reduce dimensionality
        self.rrr_pca = PCA(n_components=self.n_components)
        self.rrr_pca.fit(rrr_lm_scores)

        # project ridge betas onto PCA components to get reduced rank betas
        self.beta_proj = self.rrr_lm.coef_.T @ self.rrr_pca.components_.T
        # the reconstituted reduced rank regression matrix (same size as b_ridge)
        self.beta_rrr = self.beta_proj @ self.rrr_pca.components_
        # compute pca explained variance per component
        self.pca_explained_variance = self.rrr_pca.explained_variance_ratio_
        # compute rrr explained variance per component
        # Compute the variance of each latent score
        latent_scores = rrr_lm_scores @ self.rrr_pca.components_.T  # shape: n_samples x n_components
        var_per_component = latent_scores.var(axis=0)
        # Compute the total variance in y:
        total_var = fmri_concat.var(axis=0).sum()
        # Compute the explained variance ratio for each component
        self.rrr_explained_var_ratio = var_per_component / total_var
        ...
        return RRRResults(
            ...
            rrr_beta_rrr=self.beta_rrr,
            rrr_pca_scores=latent_scores,
            all_reg_cols=self.all_reg_cols,
            trial_types=self.trial_types,
